...
stage: private
hq: San Francisco, CA
sources: [Meow Technologies launches the first agentic banking platform for AI agents.md, Modern banking for AI Agents.md]
created: 2026-04-25
updated: 2026-05-03
---
...
## Full MCP rail support (as of May 2026)

From the live MCP page at meow.com/mcp:

| Rail | Availability |
|------|-------------|
| ACH | Domestic inbound + outbound |
| Wire Transfer | Domestic |
| International Wire | Cross-border |
| Book Transfer | Internal |
| USDC — Ethereum | Inbound + outbound |
| USDC — Base | Inbound + outbound |
| USDC — Solana | Inbound + outbound |
| USDC — Arbitrum | Inbound + outbound |
| USDT — Ethereum | Inbound + outbound |
| Card Payments | Yes |

**All networks support inbound + outbound transfers.** Meow invoices generated by agents automatically include USDC wallet addresses across Ethereum, Base, and Solana when crypto payment is enabled.

## Banking partner

**Grasshopper Bank, N.A.; Member FDIC**. Meow Technologies is a financial technology company, not itself a bank. This answers the previously open question about which FDIC-insured institution backs Meow accounts.

## MCP onboarding flow

Agent-initiated account onboarding: single CLI command or prompt paste. Supported environments: Claude Code, Cursor, Windsurf, Codex, Claude Desktop, ChatGPT, Gemini, Grok. Docs live at `meow.com/skills.md` — a machine-readable endpoint agents can read directly for setup instructions.

Key MCP tools exposed:
- `get_account_balances` — current and historical balances
- `list_bank_accounts` — all accounts across entities
- `list_account_transactions` — transaction history with timestamps
- `validate_routing_number` — confirms Fed database status, wire/ACH support, bank name/location
- `list_account_payment_networks` — shows enabled rails per account
- `list_invoices` / `list_invoicing_customers` — invoice management with crypto options
...
- What is Meow's pricing model — per transaction, per API call, or subscription?

## Sources

- [[meow-technologies-agentic-banking-launch]]
- [[modern-banking-for-ai-agents-meow]]
